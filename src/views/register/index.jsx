...
const Register = () => {
    ...
    const verifyForm = () => {
        ...
        setRegisterForm(prev => ({...prev, userErrTip: userErrTip, pwdErrTip: pwdErrTip, secPwdErrTip: secPwdErrTip}))
        return true;
    }
    const goToLogin = () => {
        history.push('/login')
    ...
    const doSubmit = async () => {
        ...
        let res = await doRegister({ username: registerForm.username, password: registerForm.password })
        if(!res.data.success){
            //TODO 待處理
        }else{
            history.replace('/')
            ...
            <div className="w-full max-w-md">
                <form className="bg-white shadow-md rounded px-8 pt-8 pb-8 mb-4">
                    ...
                    <div className="mb-7 flex items-center relative">
                        ...
                        <input 
                            className={`${registerForm.userErrTip ? 'border-red-500' : ''} shadow text-sm appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline`} 
                            id="username" 
                            type="text"
                        ...
                        />
                        {registerForm.userErrTip ? <p className="absolute -bottom-5 ml-3 left-20 text-red-500 text-xs ">請輸入帳號</p> : <></>}
                    </div>
                    <div className="mb-6 flex items-center relative">
                        ...
                        <div className="relative w-full">
                            <input 
                                className={`${registerForm.pwdErrTip ? 'border-red-500' : ''} shadow appearance-none text-sm border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline`}
                                id="password" 
                        ...
                        {registerForm.pwdErrTip ? <p className="absolute ml-3 -bottom-5 left-20 text-red-500 text-xs ">請輸入密碼</p> : <></>}
                    </div>
                    <div className="mb-6 flex items-center relative">
                        ...
                        <div className="relative w-full">
                            <input 
                                className={`${registerForm.secPwdErrTip ? 'border-red-500' : ''} shadow text-sm appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline`} 
                                id="confirmPassword" 
                        ...
                        {registerForm.secPwdErrTip ? <p className="absolute -bottom-5 left-20 ml-3 text-red-500 text-xs ">請輸入確認密碼</p> : <></>}
                    </div>
